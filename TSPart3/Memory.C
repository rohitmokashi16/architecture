...
// converts a value in memory to a string starting at address
std::string Memory::readString(int32_t address) {
  // place holder for the staring we're about to produce
  std::string result = "";
  // the starting byte in the word, eg starting at byte 3 of a 32 bit unsigned integer memory cell
  int32_t startByte = address % 4;
  // the memory cell we're using in the mem array
  int32_t startIndex = address / 4;

  // read until we encounter the end of string character
  while(true) {
    uint8_t * wordBytes = Tools::getBytes(mem[startIndex]);
    for(int i = startByte; i < 4; i++) {
      // we're at the end of the string and we can stop reading.
      if(wordBytes[i] == 0x00) {
        delete wordBytes;
        
        return result;
      }
      // append the retrieved character to the word
      char c = (char) wordBytes[i];
      result += c; 
    }
    // replace the starting byte of the word at 0
    startByte = 0;
    // increment to the next cell
    startIndex++;
    // make sure to delete the 4 bytes not to cause a memory leak
    delete wordBytes;
  }
}
/**
...
// this will pop if the address is not 4 byte aligned, I'll fix it later
uint32_t Memory::getWord(int32_t wordNum) {
  return mem[wordNum / 4]; // assuming we're 4 byte aligned
}
